...
class CustomLogger implements Reporter {
  onBegin(config: any, suite: Suite) {
    logger.info(`🔷 Test run started with ${suite.allTests().length} tests`);
  }

  onTestBegin(test: TestCase) {
    logger.info(`➡️  Starting test: ${test.title}`);
  }

  onTestEnd(test: TestCase, result: TestResult) {
    logger.info(`✅ Finished test: ${test.title} - Status: ${result.status}`);
  }
